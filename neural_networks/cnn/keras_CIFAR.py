"""
The following contains a simple implementation of a CNN with Keras,
to solve the CIFAR dataset.
"""
...
def train(model, train_images, train_labels, validation_images, validation_labels, batch_size, num_epochs, learning_rate, verbose):
    ...
    history = History(validation_images, validation_labels)
    model.fit(
        train_images,
        train_labels,
...
def predict(model, image_idx):
    ...
    plot_sample(dataset['test_images'][image_idx], classes[dataset['test_labels'][image_idx]],
                classes[pred])
